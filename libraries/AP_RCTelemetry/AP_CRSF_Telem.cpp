...
void AP_CRSF_Telem::setup_custom_telemetry() {
    gcs().send_text(MAV_SEVERITY_DEBUG,"CRSF custom telemetry enabled");
...
void AP_CRSF_Telem::update_custom_telemetry_rates(uint8_t rf_mode) {
    ...
    frsky->set_scheduler_entry_min_period(AP_Frsky_SPort_Passthrough::GPS_LAT, 100000);  // MIN Hz
    frsky->set_scheduler_entry_min_period(AP_Frsky_SPort_Passthrough::GPS_LON, 100000);  // MIN Hz
    frsky->set_scheduler_entry_min_period(AP_Frsky_SPort_Passthrough::TEXT, 100000);     // MIN Hz
}
...
bool AP_CRSF_Telem::_get_telem_data(AP_RCProtocol_CRSF::Frame* data)
{
    ...
    if (now_ms - _crsf_packet_stats_time > 5000) {
        gcs().send_text(MAV_SEVERITY_DEBUG,"CRSF rf mode: %d, packet rate %d", _telem_rf_mode, _scheduler.avg_packet_rate);
