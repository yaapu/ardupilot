...
bool FRSky::parse_sport_telemetry_data(uint8_t data, sport_parse_state_t &parse_state)
{
    switch (parse_state.state) {
        case STATE_DATA_START:
            if (data == FRAME_HEAD) {
                parse_state.state = STATE_DATA_IN_FRAME ;
        ...
        case STATE_DATA_IN_FRAME:
            if (data == FRAME_DLE) {
                parse_state.state = STATE_DATA_XOR; // XOR next byte
        ...
        case STATE_DATA_IDLE:
            if (data == FRAME_HEAD) {
                parse_state.telemetry_rx_buffer_count = 0;
                parse_state.state = STATE_DATA_START;
            }
            break;
    ...
    if (parse_state.telemetry_rx_buffer_count >= SPORT_PACKET_SIZE) {
        parse_state.telemetry_rx_buffer_count = 0;
        parse_state.state = STATE_DATA_IDLE;
        return true;
    }
    return false;
}
 ...
 */
bool FRSky::check_sport_packet(const uint8_t *packet)
{
    return check_sport_packet(packet, nullptr);
}

bool FRSky::check_sport_packet(const uint8_t *packet, const uint8_t *last_packet)
{
    ...
    if (last_packet != nullptr) {
        ...
        for (uint8_t i=1; i<SPORT_PACKET_SIZE; ++i) {
            if (packet[i] != last_packet[i]) {
                equal = false;
                // update last packet
                memcpy(last_packet,packet,SPORT_PACKET_SIZE);
                break;
            }
        }
        if (equal) {
...
void  FRSky::send_sport_frame(AP_HAL::UARTDriver *port, uint8_t frame, uint16_t appid, uint32_t data)
{
     ...
     */
    uint64_t tend = port->receive_time_constraint_us(1);
