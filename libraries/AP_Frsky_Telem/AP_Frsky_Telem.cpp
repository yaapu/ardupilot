                          ...
                          const uint32_t *ap_valuep)
{
    ...
    if ((_port = serial_manager.find_serial(AP_SerialManager::SerialProtocol_FrSky_D, 0))) {
        ...
        _passthrough.packet_period[0] = 40;   // 0x5000 status text, this weight is dynamic, when there are pending messages it changes to 50 to serve them fast
        _passthrough.packet_period[1] = 80;   // 0x5006 Attitude and range
        _passthrough.packet_period[2] = 500;   // 0x800 GPS lat
        _passthrough.packet_period[3] = 500;   // 0x800 GPS lon
        _passthrough.packet_period[4] = 500;   // 0x5005 Vel and Yaw
        _passthrough.packet_period[5] = 600;   // 0x5001 AP status
        _passthrough.packet_period[6] = 600;   // 0x5002 GPS Status
        _passthrough.packet_period[7] = 600;   // 0x5004 Home
        _passthrough.packet_period[8] = 1100;  // 0x5008 Battery 2 status
        _passthrough.packet_period[9] = 1100;  // 0x5003 Battery 1 status
        _passthrough.packet_period[10] = 1700; // 0x5007 parameters
...
uint16_t AP_Frsky_Telem::calc_avg_polling_period(uint8_t polling_period)
{
    // initialize
    if (_passthrough.avg_polling_period == 0) _passthrough.avg_polling_period = polling_period;
    // moving average
    _passthrough.avg_polling_period = (uint16_t)_passthrough.avg_polling_period * 0.9 + polling_period * 0.1;
    return _passthrough.avg_polling_period;
}
...
void AP_Frsky_Telem::send_SPort_Passthrough(void)
{
    ...
    if ((prev_byte == START_STOP_SPORT) && (_passthrough.new_byte == SENSOR_ID_28)) { // byte 0x7E is the header of each poll request
        ...
        // update average polling period
        if (_passthrough.last_poll_timer > 0) {
            calc_avg_polling_period(poll_now - _passthrough.last_poll_timer);
        }
...
bool AP_Frsky_Telem::get_next_msg_chunk(void)
{
    ...
    if (_passthrough.avg_polling_period > 58) {
        // with 3 or more extra frsky sensors on the bus
        // send messages only once
        extra_chunks = 0;
    } else if (_passthrough.avg_polling_period > 32) {
