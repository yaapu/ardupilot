...
void send_sport_frame(int sock, uint8_t frame, uint16_t appid, uint32_t data)
{
        ...
        if (parse_sport_telemetry_data(buffer[i], sport_parse_state)) {
            ...
            if (should_process_sport_packet(sport_parse_state.telemetry_rx_buffer)) {
                ...
                if (buffer[2] == 0x32) {
                    ...
                    printf("MAV:  recv msgid=%d, txcount=%u, rxcount=%u\n", rxmsg.msgid, mav_tx_count, mav_rx_count++); 
                    }
                } else {
...
void send_data() {
    if (tx_queue.empty()) {
        ...
        printf("MAV:  send msgid=%d, txcount=%u, rxcount=%u\n", txmsg.msgid, mav_tx_count++, mav_rx_count); 
    }
    printf("0x%02X: send: ", poll_frame[poll_frame_idx]); 
    if (send_frame == 0) {
        if (!tx_queue.empty()) {
            sport_packet_t packet = tx_queue.front();
            send_sport_frame(sock_handle, 0x30, packet.appid, packet.data);
            tx_queue.pop();
            tx_count++;
        }
    } else {
...
int main(int argc, char** argv)
{
    ...
    if (connect(sock_handle, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) 
    { 
        printf("\nConnection Failed \n"); 
        return -1; 
    } 
