...
const AP_Param::GroupInfo AP_MSP::var_info[] = {
    ...
    // @Bitmask: 0:EnableTelemetryMode, 1:InvertedWindPointer
    // @User: Standard
    AP_GROUPINFO("_OPTIONS", 31, AP_MSP, options, 0),
...
bool AP_MSP::init()
{
    ...
    for (uint8_t i=0; i<MSP_MAX_INSTANCES; i++) {
        if ((msp_port[i].uart = serial_manager.find_serial(AP_SerialManager::SerialProtocol_DJI_FPV, msp_idx[BACKEND_DJI]))) {
            ...
            // telemetry mode enabled by option, default is disabled
            msp_port[i].telem_backend = new AP_MSP_Telem_DJI(*this, i, check_option(AP_MSP::OPTION_TELEMETRY_MODE));
...
bool AP_MSP::check_option(uint32_t option)
{
