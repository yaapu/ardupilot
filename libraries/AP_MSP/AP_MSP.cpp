...
const AP_Param::GroupInfo AP_MSP::var_info[] = {
    ...
    // @Range: 0 15
    AP_SUBGROUPINFO(rssi, "_OSD_RSSI", 3, AP_MSP, AP_OSD_Setting),
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(alt, "_OSD_ALT", 4, AP_MSP, AP_OSD_Setting),
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(vbat, "_OSD_VBAT", 5, AP_MSP, AP_OSD_Setting),
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(curr, "_OSD_CURR", 6, AP_MSP, AP_OSD_Setting),
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(mah, "_OSD_MAH", 7, AP_MSP, AP_OSD_Setting),
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(nsats, "_OSD_SATS", 8, AP_MSP, AP_OSD_Setting),
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(flmode, "_OSD_FMODE", 9, AP_MSP, AP_OSD_Setting),
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(name, "_OSD_MSG", 10, AP_MSP, AP_OSD_Setting),
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(gspd, "_OSD_GSPD", 11, AP_MSP, AP_OSD_Setting),
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(ahor, "_OSD_HORIZ", 12, AP_MSP, AP_OSD_Setting),
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(hdist, "_OSD_HDIST", 13, AP_MSP, AP_OSD_Setting),    
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(hdir, "_OSD_HDIR", 14, AP_MSP, AP_OSD_Setting),    
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(lat, "_OSD_LAT", 15, AP_MSP, AP_OSD_Setting),
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(lon, "_OSD_LON", 16, AP_MSP, AP_OSD_Setting),
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(roll, "_OSD_ROLL", 17, AP_MSP, AP_OSD_Setting),
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(pitch, "_OSD_PITCH", 18, AP_MSP, AP_OSD_Setting),
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(battusg, "_OSD_BATT", 19, AP_MSP, AP_OSD_Setting),
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(vario, "_OSD_VSPD", 20, AP_MSP, AP_OSD_Setting),
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(arm, "_OSD_ARM", 21, AP_MSP, AP_OSD_Setting),
    ...
    // @Description: Vertical position on screen
    // @Range: 0 15
    AP_SUBGROUPINFO(cell, "_OSD_VCELL", 22, AP_MSP, AP_OSD_Setting),

    // @Param: _OSD_ASPD_EN
    // @DisplayName: _OSD_ASPD_EN
    // @Description: Displays airspeed (you'll loose pitch angle display if you enable airspedd for we recicly the osd item)
    // @Values: 0:Disabled,1:Enabled

    // @Param: _OSD_ASPD_X
    // @DisplayName: _OSD_ASPD_X
    // @Description: Horizontal position on screen
    // @Range: 0 27

    // @Param: _OSD_ASPD_Y
    // @DisplayName: _OSD_ASPD_Y
    // @Description: Vertical position on screen
    // @Range: 0 15
...
void AP_MSP::loop(void)
{
    ...
    bool update_blinking;
    while (true) {
        hal.scheduler->delay(1);

        // blink timer
        uint32_t now = AP_HAL::millis();
        update_blinking = false;
        // toggle blinking every 0.7 seconds
        // parameter to tune this one??
        if ((now - blink_start_time) > 700) {
            blink_on = !blink_on;
            blink_start_time = now;
            update_blinking = true;
        }
        ...
        for (uint8_t i=0; i< MSP_MAX_INSTANCES; i++) {
            if (msp_port[i].telem_backend != nullptr) {
                // dynamically hide/unhide only if blink status changed
                if (update_blinking) {
                    msp_port[i].telem_backend->blink_osd_items();
                }
