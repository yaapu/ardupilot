...
private:
    ...
    AP_OSD_Setting rssi     {true, 18, 0};       // OSD_RSSI
    AP_OSD_Setting vbat     {true, 1, 6};        // OSD_MAIN_BATT_VOLTAGE
    ...
    AP_OSD_Setting flmode   {true, 17, 15};      // OSD_FLYMODE - not used for now
    AP_OSD_Setting name     {true, 15, 14};      // OSD_CRAFT_NAME
    AP_OSD_Setting curr     {true, 1, 12};       // OSD_CURRENT_DRAW
    AP_OSD_Setting mah      {true, 9, 15};       // OSD_MAH_DRAWN
    AP_OSD_Setting gspd     {true, 1, 8};        // OSD_GPS_SPEED
    AP_OSD_Setting nsats    {true, 1, 0};        // OSD_GPS_SATS
    AP_OSD_Setting alt      {true, 25, 9};       // OSD_ALTITUDE
    AP_OSD_Setting cell     {true, 1, 4};        // OSD_AVG_CELL_VOLTAGE
    AP_OSD_Setting lon      {true, 9, 0};        // OSD_GPS_LON
    AP_OSD_Setting lat      {true, 9, 1};        // OSD_GPS_LAT
    AP_OSD_Setting pitch    {true, 26, 7};       // OSD_PITCH_ANGLE
    AP_OSD_Setting roll     {true, 26, 8};       // OSD_ROLL_ANGLE
    AP_OSD_Setting yaw      {true, 26, 6};       // OSD_NUMERICAL_HEADING
    AP_OSD_Setting battusg  {true, 9, 15};       // OSD_MAIN_BATT_USAGE
    AP_OSD_Setting hdir     {true, 1, 2};        // OSD_HOME_DIR
    AP_OSD_Setting hdist    {true, 1, 1};        // OSD_HOME_DIST
    AP_OSD_Setting vario    {true, 27, 10};      // OSD_NUMERICAL_VARIO
    AP_OSD_Setting arm      {true, 11, 8};       // OSD_DISARMED
    AP_OSD_Setting power    {true, 1, 13};       // OSD_POWER
    AP_OSD_Setting rtc      {true, 12, 0};       // OSD_RTC_DATETIME
    AP_OSD_Setting esctemp  {true, 6, 15};       // OSD_ESC_TMP
