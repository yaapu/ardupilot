...
#define MSP_ARRAYLEN(x) (sizeof(x) / sizeof((x)[0]))
#define MSP_ARRAYEND(x) (&(x)[MSP_ARRAYLEN(x)])
#define MSP_UNUSED(x) (void)(x)
// betaflioght/src/main/msp/msp_serial.c
#define JUMBO_FRAME_SIZE_LIMIT 255
// betaflioght/src/main/msp/msp.h
...
typedef struct msp_port_s {
    ...
    AP_MSP_Telem_Backend *telem_backend;
...
uint8_t msp_serial_checksum_buf(uint8_t checksum, const uint8_t *data, int len);
int msp_serial_send_frame(msp_port_t *msp, const uint8_t * hdr, int hdr_len, const uint8_t * data, int data_len, const uint8_t * crc, int crc_len);
int msp_serial_encode(msp_port_t *msp, msp_packet_t *packet, msp_version_e msp_version);
