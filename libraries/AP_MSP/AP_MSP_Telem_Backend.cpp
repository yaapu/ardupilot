...
//#define ENABLE_MSG_FLASH_ON_FAILSAFE
...
void AP_MSP_Telem_Backend::telem_update_flight_mode(telemetry_info_t &_telemetry)
{
    ...
    // simple mode
    if (simple_mode) {
        strncpy(buffer_ptr, " [S]",4);
        buffer_ptr += 4;
    } else if (supersimple_mode) {
        strncpy(buffer_ptr, " [SS]",5);
        buffer_ptr += 5;
    }
...
void AP_MSP_Telem_Backend::telem_update_wind(telemetry_info_t &_telemetry)
{
    ...
    AP_MSP* msp = AP::msp();
    if (msp->check_option(AP_OSD::OPTION_INVERTED_WIND)) {
        v = -v;
    }
    // if needed convert m/s to ft/s
    float v_length =  msp->units > 0 ? v.length() : v.length() * 3.28084;
    const char* unit = msp->units > 0 ? "m/s" : "f/s";
    // clear
    memset(_telemetry.flight_mode_str,0,MSP_TXT_BUFFER_SIZE+1);
    if (v_length > 1.0f) {
        ...
        // example: MANU 4m/s ↗ 
        int8_t len = snprintf(_telemetry.flight_mode_str, MSP_TXT_VISIBLE_CHARS-1, "%s %d%s ", notify->get_flight_mode_str(),  (uint8_t)roundf(v_length), unit);
        ...
        int8_t len = snprintf(_telemetry.flight_mode_str, MSP_TXT_VISIBLE_CHARS, "%s ---%s", notify->get_flight_mode_str(), unit);
        // UTF8 arrow bytes 0xE286nn
        _telemetry.flight_mode_str[len] = 0xE2;
        _telemetry.flight_mode_str[len+1] = 0x86;
        _telemetry.flight_mode_str[len+2] = arrows[0];
    }
}
...
#ifdef ENABLE_MSG_FLASH_ON_FAILSAFE
...
#endif //ENABLE_MSG_FLASH_ON_FAILSAFE
