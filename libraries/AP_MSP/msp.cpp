...
uint8_t MSP::msp_serial_checksum_buf(uint8_t checksum, const uint8_t *data, int len)
{
    while (len-- > 0) {
        checksum ^= *data++;
    }
...
int MSP::msp_serial_send_frame(msp_port_t *msp, const uint8_t * hdr, int hdr_len, const uint8_t * data, int data_len, const uint8_t * crc, int crc_len)
{
    if (msp->uart == nullptr) {
        return 0;
    }
...
int MSP::msp_serial_encode(msp_port_t *msp, msp_packet_t *packet, msp_version_e msp_version)
{
    static const uint8_t msp_magic[MSP_VERSION_COUNT] = MSP_VERSION_MAGIC_INITIALIZER;
    const int data_len = sbuf_bytes_remaining(&packet->buf);
    uint8_t hdr_buf[16] = { '$', msp_magic[msp_version], static_cast<uint8_t>(packet->result == MSP_RESULT_ERROR ? '!' : '>')};
    uint8_t crc_buf[2];
    int hdr_len = 3;
    int crc_len = 0;
...
#define V1_CHECKSUM_STARTPOS 3
    if (msp_version == MSP_V1) {
        ...
        const int v1_payload_size = sizeof(msp_header_v2_t) + data_len + 1;  // MSPv2 header + data payload + MSPv2 checksum
